...
# emits no stdout — zero in-session chatter. Scans the assistant's final
# message for banned hedge/sycophancy phrases and appends hits to a tuning log.
# Toggle off without disabling the plugin: touch ~/.claude/tone-hook.OFF
#
# Stdin: JSON. Prefers .last_assistant_message (present in current Claude Code);
# falls back to parsing the transcript for older versions.
# Stdout: nothing (always exit 0).
set -euo pipefail
...
# Preferred: the final assistant message is handed to us directly.
assistant_text=$(echo "$HOOK_INPUT" | jq -r '.last_assistant_message // ""' 2>/dev/null || echo "")

# Fallback: reconstruct the turn's assistant text from the transcript.
